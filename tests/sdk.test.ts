...
test('evmSign sends certificate to crossSignUrl and returns result', async (t) => {
...
test('evmSign throws FastError on cross-sign error response', async (t) => {
  const originalFetch = globalThis.fetch;
...
test('evmSign throws FastError on HTTP error', async (t) => {
  const originalFetch = globalThis.fetch;
...
test('executeDeposit sends approve + deposit transaction for ERC-20', async (t) => {
...
test('executeDeposit skips approve when allowance is sufficient', async (t) => {
  let approveCallCount = 0;
...
test('executeDeposit throws FastError on reverted transaction', async (t) => {
  const mockClients = {
    walletClient: {
      sendTransaction: async () => TX_HASH,
...
test('executeDeposit throws FastError on invalid receiver address', async (t) => {
  const mockClients = {
...
test('executeIntent performs 2 Fast submits + 2 cross-signs + 1 relayer call', async (t) => {
  const originalFetch = globalThis.fetch;
  const urls: string[] = [];
...
test('executeIntent uses fastBridgeAddress as recipient in TokenTransfer', async (t) => {
  const originalFetch = globalThis.fetch;
...
test('executeIntent sends correct relayer payload', async (t) => {
  const originalFetch = globalThis.fetch;
...
test('executeIntent infers external_address from Execute intent target', async (t) => {
  const originalFetch = globalThis.fetch;
...
test('executeIntent throws FastError when no external address can be resolved', async (t) => {
  const originalFetch = globalThis.fetch;
...
test('executeIntent throws FastError when intents array is empty', async (t) => {
  await assert.rejects(
    () => executeIntent({
      fastBridgeAddress: FAST_BRIDGE_ADDRESS,
      relayerUrl: RELAY_URL,
...
test('executeIntent preserves upstream FastError from fastWallet.submit', async (t) => {
...
test('executeIntent throws FastError on relayer failure', async (t) => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (url) => {
    if (String(url).includes('/relay')) {
